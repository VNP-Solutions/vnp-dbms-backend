...
export class ProcessorService implements IProcessorService {
  ...
  async remove(id: string, password: string, replacementId: string, user: IUserWithPermissions) {
    ...
    if (id === replacementId) throw new BadRequestException('Replacement processor must be different from the one being deleted')

    const item = await this.repo.findById(id)
    if (!item) throw new NotFoundException('Processor not found')

    const replacement = await this.repo.findById(replacementId)
    if (!replacement) throw new NotFoundException('Replacement processor not found')

    await this.prisma.$transaction(async (tx) => {
      await tx.property.updateMany({ where: { expedia_processor_id: id }, data: { expedia_processor_id: replacementId } })
      await tx.property.updateMany({ where: { booking_processor_id: id }, data: { booking_processor_id: replacementId } })
      await tx.property.updateMany({ where: { agoda_processor_id: id }, data: { agoda_processor_id: replacementId } })
      await tx.processor.delete({ where: { id } })
    })
